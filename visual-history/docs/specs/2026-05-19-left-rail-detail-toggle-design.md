...
- 从上到下三个组件：
  1. **顶部工具区**（高 ~60px）：Toggle 胶囊开关 `简略 │ 详尽` + 拖拽手柄提示
  2. **鸟瞰条**（高 60px，含 SVG 50px + hint 文字 10px）：公元年份线性映射，详见 §4
  3. **嵌套脊柱**（撑满剩余高度，自动滚动，当前 era 自动 scrollIntoView center）：详见 §3

> **mockup 验证后调整：** 鸟瞰条最初设计 36px，critique 显示视觉权重不足，提升到 50px SVG + 10px 底部 hint（"公元 前 2700 — 1949"）；脊柱默认自动滚动到当前 era 居中，避免详尽态首屏看不到自己在哪。
...
2. **并立支线**（三国/南北朝/五代十国-辽/宋-辽-西夏-金）
   ...
   - 该 era 行下展开 N 行支线，每行 = 一个政权（按 regimeIds 顺序自上而下）
   - 颜色取该 regime 自身的 `color` 字段
   - **Phase 1（本期）**：每行支线的时长条**等宽**（不按真实时长），只用色块 + 名字
   - **Phase 2（后续）**：在 `regimes.json` 补 `start/end` 字段后，时长条按真实年份归一化。补全清单：三国 3 条、南北朝南朝四代 + 北朝四代、五代十国共 ~14 条、宋辽夏金 4 条 ≈ 25 条记录
...
- **Hover 任意位置** → SVG 内出现 1px 虚线垂直辅助线 + 顶部 tooltip 显示"公元 X 年 · 所在 era 名"
- **点击** → 跳转 spine 到该年份所在 era（`spine.scrollToEra(eraId)`）
- **当前 era 标记**（三件套）：
  1. 顶部贯穿 2px 金条（横向跨过该 era 整个 X 范围）
  2. 中心竖向游标 2.5px 金色 + 双层 drop-shadow + pulse 2.4s
  3. 顶/底两个小三角（向内指向游标）
- 游标位置随主滚动联动（消费同一个 `onActiveEra` 事件，transform 平滑过渡 180ms）
- 底部 hint 文字"公元 前 2700  —  1949" 提示时间范围（避免用户误以为是装饰条）
...
**拖拽时的连续插值**：用户拖动到左栏 = `L`（230 ≤ L ≤ 560）期间，右栏宽度按线性插值 `R = 400 − (L − 230) × 80 / 250`，中栏 = `viewport − L − R`。即左栏每加 1px，右栏减 0.32px、中栏减 0.68px。松手吸附后取最终对应宽度。
...
**关于"虚拟父"：** 周/汉/晋/唐/宋这些没有对应 era 的父级，由前端从 eras 推导生成（凡有 ≥2 个 era 共享相同 `parentLabel`，就在脊柱里**占一行**，渲染为"父级标签行"——视觉上是这组子 era 的小标题，本身不可点击不可滚动定位）。颜色取该组首个子 era 的 `color`（如"周"取自 `western-zhou.color`）。数据保持纯净，不引入新的"伪 era"。

**父级分组的视觉包裹**（critique 后追加）：父级标签行 + 紧随其后的同 `parentLabel` 子 era + 该 era 的并立支线，**整体包在一个 `.parent-group` 容器里**，容器有横向渐变底色（左侧 4.5% alpha → 右侧透明）和右上/右下 6px 圆角。这样"周下面这一组"在视觉上是一个**横向带状区块**，而不是几行散落的文字。
...
经 mockup critique 后调优（V1 数值划线、V2 加粗）：

- 族谱竖条：~~父级 era 颜色 30% alpha / 1.5px~~ → **72% alpha / 2px，圆角**
- 父级标签的中括号装饰：父级色 85% alpha / 2.5px 竖条 + 12px 横臂 + 渐变背景包裹整个父组
- 时长条：era 自身颜色 65% alpha，圆角；当前 era 时长条 95% alpha
- 鸟瞰条段：原色 82% alpha
- 鸟瞰条当前 era：顶部贯穿金条 2px + 中心游标 2.5px 金色双层发光 (5px+9px) + pulse 2.4s + 顶/底小三角
- 当前 era 行：~~12% alpha 底色 + 2.5px 金条~~ → **22% alpha 底色 + 4px 金条 + 6px 发光 + 字重 600**
...
- **跨越断点的回滚**：localStorage 中保存的 mode 不变；窗口缩到 < 1100px 时**只在视图层**强制 light，恢复到 ≥ 1100px 时按 localStorage 还原（"用户意图"和"屏幕条件"分离）
...
**响应式：**
- [ ] < 1100px：Toggle/拖拽隐藏，强制简略态（仅视图层强制，不擦 localStorage）
- [ ] 跨越断点：1400px → 缩到 900px → 再放大到 1400px，详尽态正确恢复
- [ ] 详尽态宽度 = `clamp(420px, 32vw, 560px)`，在所有 ≥ 1100px 视口下生效

**可访问性：**
- [ ] Toggle 键盘可达，`aria-pressed` 状态正确
- [ ] `prefers-reduced-motion` 下所有过渡动画被禁用（含游标 pulse）
...
### 2026-05-19 · V1 → V2（mockup + impeccable:critique 后）

第一版 mockup（`docs/specs/mockups/full-rail.html`）经过 impeccable:critique 评估（25/36），发现 4 个 P0/P1 问题并修复，预估升级到 30/36：

| 问题 | 修复 |
|---|---|
| [P0] 鸟瞰条可发现性为 0（像装饰条） | 高度 44→60px；hover 出辅助线+tooltip；游标加 pulse+双层发光+顶底小三角；底部 hint 文字 |
| [P0] 族谱竖条 1.5px/45% 太弱 | 改为 2px/72% + 圆角；父级行加中括号样横臂 + 渐变底色包裹整个父组 |
| [P1] 当前 era 高亮太弱 | 左条 2.5→4px + 6px 发光；底色 12→22% alpha；name 字重 600；时长条 65→95% alpha |
| [P1] 52% 内容需要滚动才看到 | 详尽态打开时自动 `scrollIntoView({block:"center"})` 当前 era 到视口中部 |
